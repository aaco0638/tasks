...
public class CalculatorGame4{
    static int [] highscores = new int [5];
    static int highestAdditionScore = 0;
    static int highestTrueFalseScore = 0;
    ...
    public static void main(String[] args) {
        ...
        while (!command.equalsIgnoreCase("exit")) {// if input equals exit, it exits program.
            ...
            if (choice == 1) {
                ...
                highscores[0] = score; // stores score in array
            } else if (choice == 2) {
                score = addNewQuiz(5); // True/False quiz
                highscores[1] = score; // stores score in array
            } else {
    ...
    public static void findHighestScore(int quizType){
        int highestScore = highscores[quizType - 1]; // takes the quiz type as an argument
        System.out.println("The current highscore for this quiz is " + highestScore); // prints relevent highscore, note: can't print straight from an array so use highestScore.
    }
    ...
    public static int addQuiz(int length) {
        findHighestScore(1); //referring to this quiz being first option
        int score = 0;
...
//                score += addQuestion(i);
        ...
        if (score > highestAdditionScore){ // if score is higher than highest score saved in highestAdditionScore
            highestAdditionScore = score; //highestAdditionScore is assigned to score.
        }
...
//            boolean1 = Boolean.parseBoolean(var1);
    ...
    public static int addNewQuiz(int length) {
        ...
        findHighestScore(2); //referring to this quiz being 2nd option
        int score = 0;
...
//                score += addTrueFalse(i);
            ...
            if(score > highestTrueFalseScore){ // score is higher than highest true and false score
                highestTrueFalseScore = score; //assign score to highestTrueFalseScore
            }
